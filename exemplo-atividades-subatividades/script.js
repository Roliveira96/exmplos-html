                                ...
                                <div class="space-y-4">
                                    <div class="space-y-2.5 max-h-[220px] overflow-y-auto pr-1">
                                        ${activity.comments && activity.comments.length > 0 ? activity.comments.map(c => {
                                            ...
                                            let avatarUrl = 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=80&h=80&q=80';
                                            if (user && user.avatar) {
                                                avatarUrl = user.avatar;
                                            }
                                            return `
                                                <div class="bg-slate-950/50 border border-slate-850 p-3.5 rounded-xl text-xs flex gap-3 relative group">
                                                     <img src="${avatarUrl}" class="w-8 h-8 rounded-full object-cover flex-shrink-0 border border-slate-800/80">
                                                     <div class="flex-1 min-w-0 space-y-1">
                                                         <div class="flex justify-between items-center text-[10px] text-slate-400">
                                                             <span class="font-bold text-slate-300 font-outfit">${c.author}</span>
                                                             <span>${c.date}</span>
                                                         </div>
                                                         <p class="text-slate-200 leading-relaxed">${c.text}</p>
                                                     </div>
                                                     <button onclick="deleteComment('${phaseId}', '${activity.id}', '${c.id}')" class="absolute top-2.5 right-2.5 text-slate-600 hover:text-rose-400 opacity-0 group-hover:opacity-100 transition-opacity p-0.5 pointer-events-auto" title="Excluir comentário">
